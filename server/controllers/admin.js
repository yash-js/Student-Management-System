  ...
  try {
    ...
    await saveAdmin.save((err, admin) => {
      ...
      const mail = {
        ...
        <h4>
        <a href="http://localhost:3000/admin">Click Here To Login!</a>
        </h4>
...
console.log(4)
        ...
        if (err || !admin) {
          console.log("SDSDSDw");
          return res.status(400).json({
            error: "Something Went Wrong or User Not Found!",
          });
        }
        console.log(5)
        const link = "http://localhost:3000/admin/new-password";
        const mail = {
          to: admin.email,
          from: "yash@no-reply.com",
...
<h4><a href="http://localhost:3000">Click Here To Login!</a></h4>
